...
export function generateStaticParams() {
  return serviceSlugs.filter(slug => slug !== 'aeo').map((slug) => ({ slug }))
}
...
export default async function ServicePage({ params }: ServicePageProps) {
  const { slug, locale: routeLocale } = await params
  const { locale } = await initLocale(routeLocale)
  if (slug === 'aeo') permanentRedirect(localizedPath('/services/geo', locale))
  const service = getService(slug)
  if (!service) notFound()
