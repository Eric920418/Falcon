...
function formatPrice(service: ServiceContent): string {
  ...
  return `TWD ${new Intl.NumberFormat(locale === 'zh-hans' ? 'zh-CN' : locale).format(Number(service.priceMin))} / ${t(service.priceUnit)} ${t('起')}`
}
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="/pricing" className="falcon-btn-primary">
                {t("查看完整定價")}<ArrowRight size={18} className="ml-2 inline" />
              </Link>
              <Link
                href="/case-studies"
