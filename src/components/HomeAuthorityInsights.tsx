  ...
  return (
    <section className="bg-[#1E2A2E] px-6 py-24">
      <div className="mx-auto grid max-w-6xl gap-14 lg:grid-cols-[0.82fr_1.18fr] lg:gap-20">
        <div>
          <p className="mb-3 text-sm uppercase tracking-[0.2em] text-amber-500">{t("Named responsibility")}</p>
          <h2 className="text-3xl leading-tight text-[#E0E5E8] md:text-5xl">{t("誰負責，不用猜")}</h2>

          <div className="mt-10 border-l border-amber-600 pl-6 md:pl-8">
            <p className="text-2xl text-[#E0E5E8]">{t(primaryAuthor.name)}</p>
            <p className="mt-1 text-sm text-[#7A8A91]">{t(primaryAuthor.nameEn)}</p>
            <p className="mt-5 text-[#C5CED2]">{t(primaryAuthor.jobTitle)}</p>
            <p className="mt-4 max-w-md leading-relaxed text-[#A8B6BC]">{t(primaryAuthor.description)}</p>
          </div>
        ...
        <div>
          <div className="flex items-end justify-between gap-5 border-b border-[#344349] pb-5">
            <div>
              <p className="mb-2 text-sm text-[#7A8A91]">{t("Selected field notes")}</p>
              <h3 className="text-2xl text-[#E0E5E8] md:text-3xl">{t("先看我們怎麼判斷，再決定要不要合作")}</h3>
            </div>
            ...
            {featuredPosts.map((post, index) => (
              ...
              >
                ...
                <span>
                  ...
                  <span className="mt-2 block text-sm leading-relaxed text-[#A8B6BC]">{t(post.description)}</span>
                </span>
